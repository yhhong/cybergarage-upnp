...
public class Device implements org.cybergarage.http.HTTPRequestListener,
    ...
    public final static int DEFAULT_LEASE_TIME = 30 * 60; // @Note origin is 30 * 60
    ...
    public boolean loadDescription(File file)
        ...
        try {
            ...
            Debug.message("[Device.java] load device's description(创建设备的时候,将device's description.xml完整读取,转换为数据对象rootNode,可通过rootNode或deviceNode获取需要的子Node):\n" + rootNode.toString());
            if (rootNode == null)
                throw new InvalidDescriptionException(
    ...
    /**
     * @return
     * @Note 距离消息包发送, 消逝的时间, 比如1秒/60秒/5分钟..
     */
    ...
    /**
     * @Note 'serviceList' Node ——> ServiceList object
     * @return
     */
    ...
    /**
     * @Note ip:port/service/power/description.xml可路由到service's description.xml
     * @param searchUrl /service/power/description.xml
     * @return
     */
    ...
    /**
     * @Note 'iconList' Node ——> IconList object
     * @return
     */
    public IconList getIconList() {
        ...
        for (int n = 0; n < nNode; n++) {
            ...
            if (!Icon.isIconNode(node))
                continue;
            Icon icon = new Icon(node); //@Note 先通过deviceNode获取iconListNode,继而获取到iconNode,然后通过iconNode构造Icon
            if (icon.hasURL()) {
                String iconURL = icon.getURL();
                byte iconBytes[] = iconBytesMap.get(iconURL);//@Note iconBytesMap数据为空,未添加过
                if (iconBytes != null) {
                    icon.setBytes(iconBytes);
                } else {
                    setIconByte(icon);  // @Note add by yinghuihong
                }
            }
            iconList.add(icon);
        }
    ...
    /**
     * @Note add by yinghuihong, to set bytes into Icon object
     * @param icon
     */
    private void setIconByte(Icon icon){
        Device rootDev = getRootDevice();
        if (rootDev == null)
            return ;
        ...
        if(rootDevPath!=null) {
            File f;
            f = new File(rootDevPath.concat(iconUrl));	// @Note 该service的文件路径
            if(f.exists()) {
                icon.setBytes(File2byte(f.getPath()));
            }
        }
    }
    ...
    public void announce(String bindAddr) {
        ...
        if (isRootDevice()) {
            ...
            ssdpSock.post(ssdpReq); // @Note 这里是否该修改下 FIXME 多次重复发送??

            String devUDN = getUDN();
            ssdpReq.setNT(devUDN);
            ssdpReq.setUSN(devUDN);
            ssdpSock.post(ssdpReq); // @Note 这里是否该修改下 FIXME
        }
...
//                Debug.message("[Device.java] Notify Announce() bind address " + bindAddresses[j]);
    ...
    /**
     * @Note InetAddress array ——> String array
     */
    public void byebye() {
        InetAddress[] binds = getDeviceData().getHTTPBindAddress();
        String[] bindAddresses;
    ...
    public boolean postSearchResponse(SSDPPacket ssdpPacket, String st,
        ...
        String remoteAddr = ssdpPacket.getRemoteAddress(); // @Note 获取到Control Point所在的地址与端口号
...
//            ssdpRes.print();
    ...
    /**
     * @param httpReq client发来的http请求
     * @Note 接收到client发来的http请求;处理多种类型:远程查询控制/事件订阅
     */
    @Override
    public void httpRequestReceived(HTTPRequest httpReq) {
        ...
        if (httpReq.isSubscribeRequest()
                || httpReq.isUnsubscribeRequest()) {
            SubscriptionRequest subReq = new SubscriptionRequest(httpReq);
            deviceEventSubscriptionReceived(subReq);
            return;
        }
    ...
    private void httpGetRequestReceived(HTTPRequest httpReq) {
        ...
        Debug.message("[Device.java] httpGetRequestReceived() " + uri);
        if (uri == null) {
            httpReq.returnBadRequest();
            return;
        }
        ...
        // @Note 以下为资源路由实现
        if (isDescriptionURI(uri)) {    // @Note device's description.xml
        ...
        } else if ((embDev = getDeviceByDescriptionURI(uri)) != null) { //@Note child device's
            String localAddr = httpReq.getLocalAddress();
            contentType = XML.DEFAULT_CONTENT_TYPE;
            contentLanguage = XML.DEFAULT_CONTENT_LANGUAGE;
            fileByte = embDev.getDescriptionData(localAddr);
        } else if ((embService = getServiceBySCPDURL(uri)) != null) {   //@Note service's
            contentType = XML.DEFAULT_CONTENT_TYPE;
            contentLanguage = XML.DEFAULT_CONTENT_LANGUAGE;
            fileByte = embService.getSCPDData();
        } else if (isIconBytesURI(uri)) {                               //@Note icon
    ...
    // //////////////////////////////////////////////
    // SOAP     @Note Server角色接收到HTTP请求
    // //////////////////////////////////////////////

    private void soapBadActionReceived(HTTPRequest soapReq) {
     ...
     * @Note 远程控制命令接收处理入口
     */
    private void soapActionReceived(HTTPRequest soapReq) {
        String uri = soapReq.getURI();
        Service ctlService = getServiceByControlURL(uri);// @Note 根据URI,构建Service
        if (ctlService != null) {
            ActionRequest crlReq = new ActionRequest(soapReq);  //@Note 封装request
            deviceControlRequestReceived(crlReq, ctlService);   //@Note 接收到远程控制命令
            return;
        }
        soapBadActionReceived(soapReq);
    }

    // //////////////////////////////////////////////
    // controlAction    @Note Server角色接收到设备控制请求(也是基于HTTP+SOAP)
    // //////////////////////////////////////////////

    /**
     * @param ctlReq  请求包,封装数据
     * @param service 控制的服务
     * @Note 接收到远程控制请求, 区分查询(QUERY)与执行(ACTION)
     */
    private void deviceControlRequestReceived(ControlRequest ctlReq,
                                              Service service) {
        if (ctlReq.isQueryControl())
            deviceQueryControlReceived(new QueryRequest(ctlReq), service);
        else
            deviceActionControlReceived(new ActionRequest(ctlReq), service);
    }

    /**
     * @param ctlReq
     * @Note 响应"无效的ACTION控制"给远程控制者
     */
    private void invalidActionControlReceived(ControlRequest ctlReq) {
    ...
    /**
     * @param ctlReq
     * @Note 响应"无效的参数控制"给远程控制者
     */
    private void invalidArgumentsControlReceived(ControlRequest ctlReq) {
        ControlResponse actRes = new ActionResponse();
     ...
     * @param ctlReq
     * @param service
     * @Note 接收"远程控制"命令,根据Action
     */
    private void deviceActionControlReceived(ActionRequest ctlReq,
                                             Service service) {
//        if (Debug.isOn())
//            ctlReq.print();

        String actionName = ctlReq.getActionName();
        Action action = service.getAction(actionName);// @Note 拿到action
        if (action == null) {
            invalidActionControlReceived(ctlReq);
            return;
        }
        ...
        try {
            ...
            invalidArgumentsControlReceived(ctlReq);
            return;
        }
        if (!action.performActionListener(ctlReq)) {// @Note 执行远程控制发来的命令
            invalidActionControlReceived(ctlReq);// @Note 响应为"无效ACTION"
        }
    }
     ...
     * @param ctlReq
     * @param service
     * @Note 接收"远程查询"命令,根据StateVariable
     */
    private void deviceQueryControlReceived(QueryRequest ctlReq, Service service) {
//        if (Debug.isOn())
//            ctlReq.print();
        String varName = ctlReq.getVarName();
        if (!service.hasStateVariable(varName)) {// @Note service下是否存在该名字的StateVariable变量
            invalidActionControlReceived(ctlReq);        // @Note 否则返回"无效请求"
            return;
        }
        StateVariable stateVar = getStateVariable(varName);
        if (!stateVar.performQueryListener(ctlReq))// @Note 执行查询动作(然后通过设置StateVariable数值,并作为响应数据)
            invalidActionControlReceived(ctlReq);        // @Note 否则返回"无效请求"
    }
    ...
    private void upnpBadSubscriptionReceived(SubscriptionRequest subReq,
                                             int code) {
    ...
    private void deviceEventSubscriptionReceived(SubscriptionRequest subReq) {
        ...
        if (subReq.hasCallback() == false && subReq.hasSID() == false) {
            upnpBadSubscriptionReceived(subReq, HTTPStatus.PRECONDITION_FAILED);
            return;
        }

        // UNSUBSCRIBE
        if (subReq.isUnsubscribeRequest() == true) {
            deviceEventUnsubscriptionReceived(service, subReq);
            return;
        }

        // SUBSCRIBE (NEW)
        if (subReq.hasCallback() == true) {
            deviceEventNewSubscriptionReceived(service, subReq);
            return;
        }

        // SUBSCRIBE (RENEW)
        if (subReq.hasSID() == true) {
            deviceEventRenewSubscriptionReceived(service, subReq);
            return;
        }
    ...
    private void deviceEventNewSubscriptionReceived(Service service,
    ...
    private void deviceEventRenewSubscriptionReceived(Service service,
                                                      SubscriptionRequest subReq) {
    ...
    private void deviceEventUnsubscriptionReceived(Service service,
                                                   SubscriptionRequest subReq) {
        String sid = subReq.getSID();
        Subscriber sub = service.getSubscriber(sid);
    ...
    public boolean start() {
        ...
        // //////////////////////////////////////
        // SSDP Seach Socket    @Note 设备设置监听,接收控制点发来的搜索请求
        // //////////////////////////////////////

        SSDPSearchSocketList ssdpSearchSockList = getSSDPSearchSocketList();
        if (!ssdpSearchSockList.open())// @Note MulticastSocket.joinGroup()
            return false;
        ssdpSearchSockList.addSearchListener(this);//@Note 设置回调,收到控制点发出的搜索请求,则设备端执行该回调
        ...
        // //////////////////////////////////////
        // Announce @Note 广播"设备通知消息",首次启动,主动发出
        // //////////////////////////////////////

        announce();

        // //////////////////////////////////////
        // Advertiser   @Note 作为'广告主',不定时发出"设备通知消息"
        // //////////////////////////////////////
